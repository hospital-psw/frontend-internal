...
export class TreatmentViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.patientRelease = { description: this.medicalTreatment.report, treatmentId: this.medicalTreatment.id}
  }

  getDateString(medicalTherapy: MedicamentTherapy) {
    if (medicalTherapy.end === new Date() || !medicalTherapy.end) {
      return 'Since: ' + this.datePipe.transform(medicalTherapy.start, "dd.MM.yyyy.")
    }

    return 'From: ' + this.datePipe.transform(medicalTherapy.start, "dd.MM.yyyy.") + " To: " + this.datePipe.transform(medicalTherapy.end, "dd.MM.yyyy.");
  }
  ...
  isTextVisible(){
    return this.medicalTreatment.bloodUnitTherapies.length === 0 && this.medicalTreatment.medicamentTherapies.length === 0
  }

  releasePatient() {
    if (!this.patientRelease.description || this.patientRelease.description.length < 10){
      this.toastr.warning('Please enter at least 10 characters for report description')
      return;
    }
    this.medicalTreatmentService.releasePatient(this.patientRelease).subscribe((response) => {
      this.toastr.success('Medical treatment is successfully finished for patient ' + response.patient.firstName + " " + response.patient.lastName)
