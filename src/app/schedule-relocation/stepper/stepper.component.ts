...
export class StepperComponent implements OnInit {
  ...
  getPossibleDestinationRooms() {
    this.roomService
      .getBuilding(this.equipment.room.floor.building.id)
      .subscribe((data) => {
  ...
  filterDestinationRooms() {
    this.destinationRooms = this.destinationRooms.filter(
      (destRoom) =>
        destRoom.room.purpose !== 'hodnik' &&
  ...
  recommend() {
    ...
    this.relocationService
      .recommendDateTimes({
        roomsId: [
          this.equipment.room.id,
          this.destinationRoomForm.controls.room.value,
        ],
        fromTime: startTime1,
        toTime: endTime1,
